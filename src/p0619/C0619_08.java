...
public class C0619_08 {
	...
	public static void main(String[] args) {
		...
		Loop1: while(true) {
			...
			int choice = scan.nextInt(); // 1 -> enter키가 다음 입력칸에 영향을 미침
			scan.nextLine();
			switch(choice) {
			case 1:
				//입력화면
				while(true) {
					...
					//System.out.println("학번을 입력하세요 >>(0.취소) ");
					 ...
					 // 학번자동부여
					 // S0001
					stuNo[s_count] = "S"+String.format("%04d", s_count+1); // => 클래스 변수에서 다른 번호 부여하는 방법참조!
...
//					score[s_count][2] = scan.nextInt();
			...
			case 3:
				...
				if(temp_no==-1) { // 찾는 학생이 없을 경우
					...
					System.out.println("-----------------------------------");
					System.out.println("원하는 번호를입력하세요");
					choice = scan.nextInt();
