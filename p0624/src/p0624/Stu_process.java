...
public class Stu_process {
	int s_count = 0; // Stu_score count 사용
//	s_count = Stu_score.count;
	...
	// 학생성적입력부분
	void stu_input(Stu_score[] sc) {  // 참조형 매개변수 -> 동기화
		while(true) {
			s_count = Stu_score.count;
	...
	void stu_output(Stu_score[] sc) {
		...
		for(int i=0;i<s_count;i++) {
			...
			System.out.printf("%s\t",sc[i].kor);   // 국어
			System.out.printf("%s\t",sc[i].eng);   // 영어
			System.out.printf("%s\t",sc[i].math);  // 수학
			System.out.printf("%s\t",sc[i].total); // 합계

			System.out.printf("%.2f\t",sc[i].avg); // 평균
			System.out.printf("%s\t",sc[i].rank);  // 등수
			System.out.println();
		}
		System.out.println();
